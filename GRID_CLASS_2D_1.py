...
Use Scan to implement the grid
...
def param_init_grid3(options, params, prefix='lstm'):
    ...
    U1 = numpy.concatenate([random_weight_NS(2 * options['dim_hidden'], options['dim_hidden']),
                            random_weight_NS(2 * options['dim_hidden'], options['dim_hidden']),
                            random_weight_NS(2 * options['dim_hidden'], options['dim_hidden']),
                            random_weight_NS(2 * options['dim_hidden'], options['dim_hidden'])], axis=1)
...
def param_init_ff(options, params, prefix="lstm"):
    ...
    # W_ff_h4 = 0.1 * numpy.random.randn(options['dim_ff_hidden3'],
    #                            options['dim_ff_hidden4']).astype(config.floatX)
...
def grid_lstm_cube(tparams, origin_data, options, prefix='lstm', mask=None):
    ...
    for i in range(size_1):
        ...
        for j in range(size_2):
            ...
            h_list_sides = tensor.stack([h1, h2])[0]
                #print h_list_sides.ndim
            h_list_all[i].append(h_list_sides)
            c_list_sides = tensor.stack([c1, c2])[0]
            c_list_all[i].append(c_list_sides)
    
    out_list = [h_list_all[i][-1][1] for i in range(size_1)] # h_list_all: first three are cube index. last is the output dimension of that block, from 0 to 2.
    out_list_1 = [h_list_all[i][-1][0] for i in range(size_1)] 
    out_list_c = [c_list_all[i][-1][1] for i in range(size_1)] 
    out_list_c1 = [c_list_all[i][-1][0] for i in range(size_1)] 
    
    print 'every h to stack is in dim: %d' %(h_list_all[-1][-1][1].ndim)
    proj = tensor.stack(out_list)[0]
    proj_1 = tensor.stack(out_list_1)[0]
    proj_c = tensor.stack(out_list_c)[0]
    proj_c1 = tensor.stack(out_list_c1)[0]
    proj_final = tensor.concatenate([proj, proj_1, proj_c, proj_c1], axis=2)
    print 'proj_final.ndim is %d' %(proj_final.ndim)
    ...
    return proj_final, all_medium_states, h_input_all
...
def grid_lstm_cube_scan(tparams, origin_data, options, prefix='lstm',mask=None):
    depth_1 = origin_data.shape[0]
    depth_1_num = options['grid_depth_1']
    depth_2 = options['grid_depth_2']
    dim_hidden = options['dim_hidden']
    if origin_data.ndim == 3:
        n_samples = origin_data.shape[1]
    else:
    ...
    def step_one(h_1, c_1, h_2, c_2, m):
        h_in = tensor.concatenate([h_1, h_2], axis=1)
        preact1 = tensor.dot(h_in, tparams[_p(prefix, 'U1')]) + tparams[_p(prefix, 'b1')]
        i1 = tensor.nnet.sigmoid(_slice(preact1, 0, options['dim_hidden']))
        f1 = tensor.nnet.sigmoid(_slice(preact1, 1, options['dim_hidden']))
    ...
    def step_column(m, data_in, stacked_h_1, stacked_c_1):
        medium_states, medium_updates = theano.scan(step_one, sequences=[stacked_h_1, stacked_c_1], outputs_info=[data_in, 
                                                                                    tensor.alloc(numpy_floatX(0.), n_samples, dim_hidden), None, None], 
                                                                                                    non_sequences=[m], 
                                                                                                    name='grid_cube_dim2', 
                                                                                                    n_steps=depth_2)
        return medium_states[2], medium_states[3], medium_states[0], medium_states[1] # h1,c1,h2,c2 at every column, number is depth_2
        
    rval, updates = theano.scan(step_column, sequences=[mask, input_data], outputs_info=[tensor.alloc(numpy_floatX(0.), 
                                                                                                    depth_2, n_samples, dim_hidden), 
                                                                                          tensor.alloc(numpy_floatX(0.), 
                                                                                                    depth_2, n_samples, dim_hidden), None, None],
                                                                                          name='grid_cube_dim1', 
                                                                                          n_steps=depth_1)
    print len(rval), rval[0].dtype, rval[0].ndim
    
    # proj_final, all_medium_states, h_input_all
    out_list_h1 = [rval[0][i][-1] for i in range(depth_1_num)]
    out_list_c1 = [rval[1][i][-1] for i in range(depth_1_num)]
    out_list_h2 = [rval[2][i][-1] for i in range(depth_1_num)]
    out_list_c2 = [rval[3][i][-1] for i in range(depth_1_num)]
    
    proj_h1 = tensor.stacklists(out_list_h1)
    proj_c1 = tensor.stacklists(out_list_c1)
    proj_h2 = tensor.stacklists(out_list_h2)
    proj_c2 = tensor.stacklists(out_list_c2)
    proj_final = tensor.concatenate([proj_h1, proj_c1, proj_h2, proj_c2], axis=2)
    print 'proj_final.ndim is %d' %(proj_final.ndim)
    
    all_medium_states_list = [rval[0], rval[2]]
    all_medium_states = tensor.stacklists(all_medium_states_list)
    all_medium_states = all_medium_states.dimshuffle(1,2,0,3,4)
    print 'all_medium_states.ndim is %d' %(all_medium_states.ndim)
    
    h_input_all = input_data
    print 'h_input_all.ndim is %d' %(h_input_all.ndim)
...
def feed_forward_stm(tparams, proj, q, options, prefix='ff',
    ...
    def ff_step(single_q, single_m, single_proj):
        ...
        # h4_t = tensor.nnet.softplus(tensor.dot(h3_t, tparams['W_ff_h4']))
...
def rmsprop(lr, tparams, grads, x, mask, y, q, cost):
    """
    ...
    f_grad_shared = theano.function([x, mask, y, q], cost,
...
def build_model_STM(tparams, options):
    ...
    proj, all_medium_states, h_input_all= grid_lstm_cube_scan(tparams, x, options,
                                            prefix=options['encoder'],
                                            mask=mask)
    print 'proj.ndim is: %d' %(proj.ndim)
    print 'proj_0.ndim is: %d' %(proj[0].ndim)
    if options['encoder'] == 'lstm':
        proj = (proj[0] * mask[:, :, None]).sum(axis=0)
...
def write_all_states(data_x, h_input_all, all_medium_states):
    fd = open('stmDebug.txt', 'w+')
    oldstd = sys.stdout
    sys.stdout = fd
    ...
    print h_input_all[:,0,:]
    print 'the h_1 medium states:\n'
    for i in range(5):
        for j in range(3):
            print 'h_1[%d, %d]:\n' %(i,j)
            print all_medium_states[i,j,0,0,:]
    print 'the h_2 medium states:\n'
    for i in range(5):
        for j in range(3):
            print 'h_2[%d, %d]:\n' %(i,j)
            print all_medium_states[i,j,1,0,:]
    fd.close()
    sys.stdout = oldstd
    

def debug_print(train_err, f_pred, f_grad, prepare_data, data, iterator, verbose=False):
    ...
    if train_err < 0.7:
        write_all_states(data_x, h_input_all, all_medium_states)


def train_lstm(
    dim_proj=10,  # data dimension
    dim_hidden=40, # lstm unit number
    dim_ff_hidden1=40, # ff hidden layer size
    dim_ff_hidden2=40,
    dim_ff_hidden3=40,
    # dim_ff_hidden4=100,
    grid_depth_1=8, # time step of input data. to be removed because scan() can handle a tensorVariable as iterator.
    grid_depth_2=3, # number of grid lstm layers on certain sides. grid_depth_1 == time steps of data for default case.
    #grid_depth_3=2,
    dim_q=8, # dim of question data
    dim_q_emb=40, # dim of question embedding
        ...
        for eidx in xrange(max_epochs):
            ...
            for _, train_index in kf:
                ...
                if numpy.mod(uidx, validFreq) == 0:
                    ...
                    debug_print(train_err, f_debug, f_grad,  prepare_data, train,kf)
