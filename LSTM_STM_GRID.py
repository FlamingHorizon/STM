'''
Build a STM with three-grid-LSTM and FeedForward Network
edited by yyq at 2016-02-29
'''
...
def param_init_grid3(options, params, prefix='lstm'):
    ...
    W = ortho_weight_NS(options['dim_proj'], options['dim_hidden'])
    params[_p(prefix, 'W')] = W
    
    # U1 to U3 is the LSTM params in each grid side. They are shared in this experiment by all blocks.
    U1 = numpy.concatenate([ortho_weight_NS(3 * options['dim_hidden'], options['dim_hidden']),
                            ortho_weight_NS(3 * options['dim_hidden'], options['dim_hidden']),
                            ortho_weight_NS(3 * options['dim_hidden'], options['dim_hidden']),
                            ortho_weight_NS(3 * options['dim_hidden'], options['dim_hidden'])], axis=1)
    params[_p(prefix, 'U1')] = U1
    print 'U1.shape:' 
    print U1.shape
    
    U2 = numpy.concatenate([ortho_weight_NS(3 * options['dim_hidden'], options['dim_hidden']),
                            ortho_weight_NS(3 * options['dim_hidden'], options['dim_hidden']),
                            ortho_weight_NS(3 * options['dim_hidden'], options['dim_hidden']),
                            ortho_weight_NS(3 * options['dim_hidden'], options['dim_hidden'])], axis=1)
    params[_p(prefix, 'U2')] = U2

    U3 = numpy.concatenate([ortho_weight_NS(3 * options['dim_hidden'], options['dim_hidden']),
                            ortho_weight_NS(3 * options['dim_hidden'], options['dim_hidden']),
                            ortho_weight_NS(3 * options['dim_hidden'], options['dim_hidden']),
                            ortho_weight_NS(3 * options['dim_hidden'], options['dim_hidden'])], axis=1)
    params[_p(prefix, 'U3')] = U3
    
    b1 = numpy.zeros((4 * options['dim_hidden'],))
    b2 = numpy.zeros((4 * options['dim_hidden'],))
    b3 = numpy.zeros((4 * options['dim_hidden'],))
    params[_p(prefix, 'b1')] = b1.astype(config.floatX)
    params[_p(prefix, 'b2')] = b2.astype(config.floatX)
    params[_p(prefix, 'b3')] = b3.astype(config.floatX)
...
def param_init_ff(options, params, prefix="lstm"):
    ...
    :see: init_params
    """
    W_ff_h1 = 0.1 * numpy.random.randn(options['dim_hidden'],
    ...
    W_ff_h3 = 0.01 * numpy.random.randn(options['dim_ff_hidden2'],
...
def grid_lstm_block(tparams, h_1, h_2, h_3, c_1, c_2, c_3, options, prefix='lstm', m=None): # m is the mask value of each sample in this time step
    #print h_1.ndim, h_2.ndim, h_3.ndim
    h_in = tensor.concatenate([h_1, h_2, h_3], axis=1)
    ...
    preact2 = tensor.dot(h_in, tparams[_p(prefix, 'U2')]) + tparams[_p(prefix, 'b2')]
    ...
    h_in_out = tensor.concatenate([h1, h2, h_3], axis=1)
    preact3 = tensor.dot(h_in_out, tparams[_p(prefix, 'U3')]) + tparams[_p(prefix, 'b3')]
    i3 = tensor.nnet.sigmoid(_slice(preact3, 0, options['dim_hidden']))
    f3 = tensor.nnet.sigmoid(_slice(preact3, 1, options['dim_hidden']))
    o3 = tensor.nnet.sigmoid(_slice(preact3, 2, options['dim_hidden']))
    c3 = tensor.tanh(_slice(preact3, 3, options['dim_hidden']))
    c3 = f3 * c_3 + i3 * c3 
    c3 = m[:, None] * c3 + (1. - m)[:, None] * c_3 # m: 1 * n_samples; m[:,None]: n_samples * 1. then broadcast to n_samples * dim_hidden to multiply elementwise with c
    h3 = o3 * tensor.tanh(c3)
    h3 = m[:, None] * h3 + (1. - m)[:, None] * h_3

    return h1, h2, h3, c1, c2, c3

def grid_lstm_cube(tparams, input_data, options, prefix='lstm', mask=None):
    # size_1 = input_data.shape[0]
    ...
    input_data = tensor.dot(input_data, tparams[_p(prefix, 'W')])
        ...
        for j in range(size_2):
            ...
            for k in range(size_3):
                ...
                if i < 1:
                    ...
                    h_1 = h_list_all[i-1][j][k][0][0]
                    c_1 = c_list_all[i-1][j][k][0][0]
                ...
                if j < 1:
                    ...
                    h_2 = h_list_all[i][j-1][k][0][1]
                    c_2 = c_list_all[i][j-1][k][0][1]

                if k < 1:
                    c_3 = tensor.alloc(numpy_floatX(0.), n_samples, dim_hidden)
                    h_3 = tensor.alloc(numpy_floatX(0.), n_samples, dim_hidden)
                else:
                    h_3 = h_list_all[i][j][k-1][0][2]
                    c_3 = c_list_all[i][j][k-1][0][2]
    ...
    out_list = [h_list_all[i][-1][-1][0][2] for i in range(size_1)]
    print 'every h to stack is in dim: %d' %(h_list_all[-1][-1][-1][0][2].ndim)
    a = tensor.stack(out_list)
    print 'a.ndim is %d' %(a.ndim)
    return tensor.stack(out_list)
...
def build_model_STM(tparams, options):
    ...
    proj = grid_lstm_cube(tparams, x, options,
                                            prefix=options['encoder'],
                                            mask=mask)
    ...
    f_debug = theano.function([x, mask, y, q], [pred, qemb_seq, proj_seq, x, mask, y, q, h1_seq, h2_seq, h3_seq], name='f_debug')
...
def debug_print(f_pred, prepare_data, data, iterator, verbose=False):
    ...
    x, mask, y, q = prepare_data([data[0][t] for t in valid_index],
                              ...
                              [data[2][t] for t in valid_index],
                              maxlen=None)
    [pred, qemb_seq, proj_seq, data_x, data_mask, data_y, data_q, h1_seq, h2_seq, h3_seq] = f_pred(x, mask, y, q)
...
def train_lstm(
    ...
    dim_ff_hidden1=100, # ff hidden layer size
    dim_ff_hidden2=100,
    dim_ff_hidden3=100,
    grid_depth_1=5, # time step of input data. to be removed because scan() can handle a tensorVariable as iterator.
    grid_depth_2=2, # number of grid lstm layers on certain sides. grid_depth_1 == time steps of data for default case.
    grid_depth_3=2,
    dim_q=5, # dim of question data
    dim_q_emb=25, # dim of question embedding
        ...
        for eidx in xrange(max_epochs):
            ...
            for _, train_index in kf:
                ...
                if numpy.mod(uidx, validFreq) == 0:
                    ...
                    debug_print(f_debug, prepare_data, train,kf)
                    print unzip(tparams)['b_ff_o']
